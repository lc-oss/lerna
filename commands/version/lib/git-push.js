...
function getPushArgsOverride(defaultArgs) {
  const { GIT_PUSH_ARGS } = process.env;

  if (GIT_PUSH_ARGS) {
    log.silly("gitPush args override provided", GIT_PUSH_ARGS);
    return ["push", ...GIT_PUSH_ARGS.split(" ")];
  }
...
function gitPush(remote, branch, opts) {
  ...
  const pushArgs = getPushArgsOverride(["push", "--follow-tags", "--no-verify", "--atomic", remote, branch]);

  return childProcess.exec("git", pushArgs, opts).catch(error => {
    ...
    if (
      ...
      pushArgs.splice(pushArgs.indexOf("--atomic"), 1);
      return childProcess.exec("git", pushArgs, opts);
    }
